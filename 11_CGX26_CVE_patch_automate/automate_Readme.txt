...
user@group:/usr/local/bin$ cp git-commit-mv git-commit-mvauto
...
+        fp = open('automate_generated_details.txt', 'r')       ## -> Added by Shubham
+        cve_summary=fp.read()
+        fp.close()
+        header += cve_summary
+        header += "\n\n"                                       ## <- Added by Shubham
...
user@group:/usr/local/bin$ patch -p1 < diff.patch
...
user@group:WorkDirectory	   $ mkdir -p gregkh-linux/4.19-linux
user@group:WorkDirectory	   $ cd gregkh-linux/4.19-linux
user@group:gregkh-linux/4.19-linux $ git clone -b linux-4.19.y https://github.com/gregkh/linux.git
user@group:gregkh-linux/4.19-linux $ ls
				   $ linux


Files that needs to be present in 'linux-mvista-2.6' dir:
i.e. automate_* files

	automate_Readme.txt
	automate_bugzpass.txt			-> Add your bugz password
	automate_commits.c
	automate_generated_details.txt
	automate_python.py
	automate_run.out
	automate_send_pull_req.py
	

user@group:WorkDirectory	   $ gcc automate_commits.c -o automate_run.out
